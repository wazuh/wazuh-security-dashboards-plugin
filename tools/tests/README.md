...
`tools/tests/tmp/` for every test case, with the minimal structure this
(single-plugin) repo has: a root `VERSION.json`, a root `package.json` with
the nested `wazuh` block, a `CHANGELOG.md` and GitHub workflow files. The
scripts under test are **copied into the fixture** and executed there, so
log files and every edit land inside the fixture, never in the real
repository.

One trick keeps everything deterministic and offline: the fixture lives in a
path containing `github.com/wazuh/test-repo` and its `origin` remote points
to that same local path. This makes `git ls-remote --tags origin` (tag
discovery) and `get_repo_path()` (link generation) work without network
access. The fixture ships a known set of tags — `4.9.x`, `4.10.x`, `v4.10.1`,
`5.0.0`, plus invalid ones like `not-a-version` — so the "Prior versions"
output is fully predictable.
...
- Full bump: `VERSION.json`, the root `package.json` `wazuh` block (version +
  revision reset to `00`, top-level template `version` untouched), changelog
  reset via `changelog_bump.sh`, and the workflow `default: main` branch ref
...
- `--tag` with and without `--stage` (refs become `vX.Y.Z[-stage]`, changelog
  only resynced).
- `--set-as-main` bumps versions but leaves the workflow branch ref alone.
